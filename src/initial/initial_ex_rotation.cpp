...
{
    frame_count_ = 0;
    Rc_.push_back(Eigen::Matrix3d::Identity());
    Rc_g_.push_back(Eigen::Matrix3d::Identity());
    Rimu_.push_back(Eigen::Matrix3d::Identity());
                                              ...
                                              Eigen::Quaterniond delta_q_imu, Eigen::Matrix3d& calib_ric_result)
{
    ...
    {
        Eigen::Quaterniond r1(Rc_[i]);
        Eigen::Quaterniond r2(Rc_g_[i]);
        ...
        double          w   = Eigen::Quaterniond(Rc_[i]).w();
        Eigen::Vector3d q   = Eigen::Quaterniond(Rc_[i]).vec();
        L.block<3, 3>(0, 0) = w * Eigen::Matrix3d::Identity() + Utility::skewSymmetric(q);
        L.block<3, 1>(0, 3) = q;
        L.block<1, 3>(3, 0) = -q.transpose();
        L(3, 3)             = w;

        Eigen::Quaterniond R_ij(Rimu_[i]);
        w                   = R_ij.w();
        q                   = R_ij.vec();
...
Eigen::Matrix3d
InitialEXRotation::solveRelativeR(const std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>>& corres)
{
    if (corres.size() >= 9)
    {
